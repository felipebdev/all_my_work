    ...
    private function readFromDataBase(Request $request)
    {
        ...
        foreach ($rows as $row)
        {
            if(isset($row->audits[0]))
            {
                ...
                {
                   if(is_array($audit->old_values) && count($audit->old_values)) $data[] = (object) $this->{$formatter}($row, $audit, 'old_values');
                   if(is_array($audit->new_values) && count($audit->new_values)) $data[] = (object) $this->{$formatter}($row, $audit, 'new_values');
                }
            }
        }

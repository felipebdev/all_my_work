...
edtPassword.addEventListener('keydown', function () {
    console.log(edtPassword.value.trim());
    let password = edtPassword.value.trim();
    // console.log((/[^a-zA-Z]/g.test(password)));
    (/[0-9]/.test(password)) ? divPolicyNumber.classList.add('active') : divPolicyNumber.classList.remove('active');
    (/[^a-zA-Z]/g.test(password)) ? divPolicyLetter.classList.add('active') : divPolicyLetter.classList.remove('active');
    (/^[!@#\$%\^\&*\)\(+=._-]+$/g.test(password)) ? divPolicySpecial.classList.add('active') : divPolicySpecial.classList.remove('active');

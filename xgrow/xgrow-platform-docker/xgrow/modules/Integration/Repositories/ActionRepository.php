...
class ActionRepository extends BaseRepository implements IActionRepository
{
    ...
    public function updateAndSyncPlans(
        ...
        $this->plansDetach($model, $plans);
        $this->plansAttach($model, $plans);
        return $model;
    }

    private function plansDetach($model, $plans){
        $currents = $model->plans()->get();
        foreach ($currents as $current){
            if(!in_array($current->id, $plans)){
                $model->plans()->detach($current->id);
            }
        }
    }

    private function plansAttach($model, $plans){
        foreach ($plans as $plan){
            if(!$model->plans()->where('plan_id', $plan)->first()){
                DB::table('app_action_products')->insert([
                        'app_action_id' => $model->id,
                        'plan_id' => $plan,
                        'product_id' => Plan::find($plan)->product_id
                ]);
            }
        }
    }

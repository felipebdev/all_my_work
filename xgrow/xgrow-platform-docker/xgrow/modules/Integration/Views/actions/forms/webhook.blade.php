@push('after-scripts')
    <script>
        $(document).ready(function() {
            $('#webhook-days_never_accessed').val('').prop('required', false).hide();
            $('#webhook-event').change(function () {
                const selectedEvent = $(this).val();
                selectedEvent === 'onNeverAccessed' ?
                    $('#webhook-days_never_accessed').prop('required', true).show() :
                    $('#webhook-days_never_accessed').val('').prop('required', false).hide();
            });
        })
    </script>
    ...
    <div class="modal-integration-wrapper">
        ...
        <form action="{{ route('apps.integrations.actions.store', ['integration' => $integration->id]) }}" method="POST">
            ...
            <div class="xgrow-form-control mb-2">
                <select class="xgrow-select slc-products" id="webhook-products"
                        onChange="changeProduct('webhook')"
                        multiple></select>
            </div>
            <div class="xgrow-form-control mb-2">
                <select class="xgrow-select slc-plans" id="webhook-plans" name="plans[]" multiple></select>
            </div>
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-2">
            ...
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-2">
                <input id="webhook-days_never_accessed" name="metadata[days_never_accessed]" type="number" min="1"
                    class="mui--is-empty mui--is-untouched mui--is-pristine">
                <label for="webhook-days_never_accessed">Dias sem acessar</label>
            </div>
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-2">
                @php
                    $actions = [

@push('after-scripts')
    <script>
        $(document).ready(function () {
            $("#memberkit-days_never_accessed").val("").prop("required", false).hide();
            $("#memberkit-event").change(function () {
                const selectedEvent = $(this).val();
                selectedEvent === "onNeverAccessed" ?
                    $("#memberkit-days_never_accessed").prop("required", true).show() :
                    $("#memberkit-days_never_accessed").val("").prop("required", false).hide();
            });
        });
    </script>
...
<div id="modal-action-memberkit" class="modal-integration modal-integration-two-items action-form">
    <div class="modal-integration-wrapper">
        ...
        <form action="{{ route('apps.integrations.actions.store', ['integration' => $integration->id]) }}"
            ...
            <div class="d-flex form-check form-switch mb-3">
                <input class="form-check-input me-2" type="checkbox" id="memberkit-is_active" name="is_active" value="1"
                       checked="">
            ...
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-3">
                <input required="" id="memberkit-description" name="description" type="text"
                       class="mui--is-empty mui--is-untouched mui--is-pristine">
                <label for="memberkit-description">Nome da ação</label>
            </div>
            <div class="xgrow-form-control mb-3">
                <select class="xgrow-select slc-products" id="memberkit-products"
                        onChange="changeProduct('memberkit')" multiple></select>
            </div>
            <div class="xgrow-form-control mb-3">
                <select class="xgrow-select slc-plans" id="memberkit-plans" name="plans[]" multiple></select>
            </div>
            ...
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-3">
                <input id="memberkit-days_never_accessed" name="metadata[days_never_accessed]" type="number" min="1"
                       class="mui--is-empty mui--is-untouched mui--is-pristine">
                <label for="memberkit-days_never_accessed">Dias sem acessar</label>
            </div>
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-3">

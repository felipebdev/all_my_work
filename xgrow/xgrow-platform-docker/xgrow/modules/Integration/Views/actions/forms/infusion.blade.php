@push('after-scripts')
    <script>
        $(document).ready(function() {
            $('#infusion-days_never_accessed').val('').prop('required', false).hide();
            $('#infusion-event').change(function () {
                const selectedEvent = $(this).val();
                selectedEvent === 'onNeverAccessed' ?
                    $('#infusion-days_never_accessed').prop('required', true).show() :
                    $('#infusion-days_never_accessed').val('').prop('required', false).hide();
            });
    ...
    <div class="modal-integration-wrapper">
        ...
        <form action="{{ route('apps.integrations.actions.store', ['integration' => $integration->id]) }}" method="POST">
            ...
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-2">
                ...
                <select class="xgrow-select slc-products" id="infusion-products"
                        onChange="changeProduct('infusion')"
                        multiple></select>
            </div>
            <div class="xgrow-form-control mb-2">
                <select class="xgrow-select slc-plans" id="infusion-plans" name="plans[]" multiple></select>
            </div>
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-2">
            ...
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-2">
                <input id="infusion-days_never_accessed" name="metadata[days_never_accessed]" type="number" min="1"
                    class="mui--is-empty mui--is-untouched mui--is-pristine">
                <label for="infusion-days_never_accessed">Dias sem acessar</label>
            </div>
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-2">
                @php
                    $actions = [

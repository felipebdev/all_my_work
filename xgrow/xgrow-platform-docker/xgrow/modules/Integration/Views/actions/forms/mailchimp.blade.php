@push('after-scripts')
    <script>
        $(document).ready(function() {
            $('#mailchimp-days_never_accessed').val('').prop('required', false).hide();
            $('#mailchimp-event').change(function () {
                const selectedEvent = $(this).val();
                selectedEvent === 'onNeverAccessed' ?
                    $('#mailchimp-days_never_accessed').prop('required', true).show() :
                    $('#mailchimp-days_never_accessed').val('').prop('required', false).hide();
            });
    ...
    <div class="modal-integration-wrapper">
        ...
        <form action="{{ route('apps.integrations.actions.store', ['integration' => $integration->id]) }}" method="POST">
            ...
            <div class="xgrow-form-control mb-2">
                <select class="xgrow-select slc-products" id="mailchimp-products"
                        onChange="changeProduct('mailchimp')"
                        multiple></select>
            </div>
            <div class="xgrow-form-control mb-2">
                <select class="xgrow-select slc-plans" id="mailchimp-plans" name="plans[]" multiple></select>
            </div>
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-2">
            ...
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-2">
                <input id="mailchimp-days_never_accessed" name="metadata[days_never_accessed]" type="number" min="1"
                    class="mui--is-empty mui--is-untouched mui--is-pristine">
                <label for="mailchimp-days_never_accessed">Dias sem acessar</label>
            </div>
            <div class="xgrow-floating-input mui-textfield mui-textfield--float-label mb-2">
                @php
                    $actions = [

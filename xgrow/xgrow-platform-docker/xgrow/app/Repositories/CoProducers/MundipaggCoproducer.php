 ...
 *
 */
class MundipaggCoproducer
{
    /**
     * @return \MundiAPILib\Controllers\RecipientsController
     */
    private static function getRecipients()
    {
        $mundipaggService = new MundipaggService(request()->route()->parameters()['platformId']);
    ...
    {
        return Producer::where('platform_user_id', Auth::user()->id)
            ->where('platform_id', request()->route()->parameters()['platformId'])
            ->first()->recipient_id;
    }
    ...
    {
        return self::getRecipients()->getWithdrawals(self::getRecipientId());
    }
    ...
    {
        foreach (self::getClientRecipient()->gatewayRecipients as $cod => $attr) {
    ...
    {
        $splitService = new SplitService(request()->route()->parameters()['platformId']);

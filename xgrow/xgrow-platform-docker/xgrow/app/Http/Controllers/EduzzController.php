...
class EduzzController extends Controller
{
    ...
    public function principal($integration_id, Request $request)
    {
        ...
        Log::info('Inicio Eduzz => ' . date('d/m/Y H:i:s'));
...
//                    case 1:
    ...
    {
        $status = ($request->type === 'contract') ? Integration::eduzzPurchaseStatus($request->recurrence_status) : Integration::eduzzInvoiceStatus($request->trans_status);
...
//            Descomentar esse trecho após o entendimento de quando uma fatura/assinatura é paga (data de pagamento está vindo null nos testes recebidos da Eduzz [campo trans_paiddate])
//            $orderIntegration = ($plan->type_plan === 'R' && $request->type === 'contract') ? $request->recurrence_cod : $request->trans_cod;
...
//            return $this->payment($platform_id, $orderId, $request);
    ...
    public function unsubscribe($platform_id, Request $request)
    {
        ...
        $status = Integration::eduzzPurchaseStatus($request->recurrence_status);

        if ($status !== Integration::STATUS_CANCELED) {
            return response()->json(['status' => 'error', 'message' => 'Não aplicável'], 204);
        }

        $integrationType = IntegrationType::whereIntegrationTypeId($request->recurrence_cod)
            ->whereIntegratableType(Subscription::class)
            ->first();

        if ($integrationType === null) {
            return response()->json(['status' => 'error', 'message' => 'Contrato não encontrado [#'.$request->recurrence_cod.']'], 204);
        }
    ...
    public function verifyPlan($request, $platform_id)
    {
        ...
        if (empty($plan)) {
            ...
            $plan->recurrence = Integration::eduzzRecurrency($request->recurrence_interval_type);
            $plan->status = 1;
            ...
            $integration = Integration::where('id_integration', '=', Constants::CONSTANT_INTEGRATION_EDUZZ)->where('platform_id', $platform_id)->first();
    ...
    private function createSubscriber($request, $platform_id, $status, $plan)
    {
        ...
        $integration = Integration::where('platform_id', $subscriber->platform_id)->where('id_integration', '=', Constants::CONSTANT_INTEGRATION_EDUZZ)->first();
        if ($integration === null) {
            $subscriber->delete();

...
@php
    //External Routes
    $externalRoutes = ['choose.platform', 'choose.platform.new', 'choose.platform.my.account', 'new.platform', 'coproducer', 'affiliations', 'affiliations.products', 'affiliations.products.resume', 'affiliations.products.transactions', 'affiliations.products.withdraws', 'documents'];
@endphp

<body class="dom-loading">
    {{-- @include('templates.xgrow.loading-dom') --}}
    <div class="d-flex" id="wrapper">
        @include('templates.xgrow.left-sidebar')
        <div id="page-content-wrapper" class="w-100">
            @include('templates.xgrow.navbar')
            @include('templates.xgrow.header')
    ...
    <script src="{{ asset('xgrow-vendor/assets/js/responsive_sidenav.js') }}"></script>
    <script src="{{ asset('/js/helpers.js') }}"></script>

    @if (env('ZENDESK_URL', false))
        <!-- Start of xgrowsupport Zendesk Widget script -->
        <script id="ze-snippet" src="{{ env('ZENDESK_URL') }}"></script>
        <!-- End of xgrowsupport Zendesk Widget script -->
    @endif

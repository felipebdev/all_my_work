<template>
    <span>
        ...
        <div class="widget-box__header d-flex align-items-center gap-3 mt-2">
            <Title icon="fas fa-layer-group" is-form-title icon-bg="transparent" icon-color="#FFFFFF">
                Grupo (esteira)
            </Title>
        </div>
        <div class="widget-box__content overflow-hidden">
            <template v-for="(widget, i) in item.groupItems" :key="i">
                <template v-if="widget.verticalImage || widget.horizontalImage">
                    <img :src="widget.horizontalImage" class="widget-img" alt="Imagem do conteúdo" />
                </template>
                <template v-else>
        ...
        <ContentGroupModal :modal="modal.active" :item="item" @close-modal="(val) => { modal.active = val }" />
    </span>
</template>
...
import ContentGroupModal from "../modals/ContentGroupModal.vue";
...
export default {
    name: "ContentGroup",
    components: { ContentGroupModal, IconButton, Subtitle, Title, Col },
    props: {
        item: { type: Object },
    ...
    mounted() {
        this.$props.item.groupItems.forEach(async item => {
            ...
            if (isCourse) {
                ...
                const index = this.$props.item.groupItems.findIndex(item => item.itemId == course.id)
                this.$props.item.groupItems[index].horizontalImage = course.horizontal_image
                this.$props.item.groupItems[index].verticalImage = course.vertical_image
            } else {
                const contentQuery = {
                ...
                const index = this.$props.item.groupItems.findIndex(item => item.itemId == content.id)
                this.$props.item.groupItems[index].horizontalImage = content.horizontal_image
                this.$props.item.groupItems[index].verticalImage = content.vertical_image
            }
        });
    }
}

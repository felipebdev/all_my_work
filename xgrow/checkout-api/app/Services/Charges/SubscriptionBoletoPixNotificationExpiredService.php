...
use App\Jobs\BoletoPixExpiredNotificationJob;
...
class SubscriptionBoletoPixNotificationExpiredService
{
    ...
    public function dispatchSingleRecurrenceNotification(Recurrence $recurrence): bool
    {
        ...
        BoletoPixExpiredNotificationJob::dispatchSync($recurrence);
    ...
    public function handleEmailNotification(Recurrence $recurrence)
    {
        ...
        if (strlen(config('app.renewal_link')) > 0) {
            $mail = new SendMailExpiredBoletoPix($recurrence);
            EmailTaggedService::mail($platformId, 'EXPIRED_BOLETO_PIX', $mail, [$recurrence->subscriber->email]);
        }
    ...
    private function saveLastRecurrenceIdProcessedToday($recurrenceId): void
    {
        ...
        Cache::put(LogKeys::CRON_NOTIFY_EXPIRED_BOLETO_PIX_LAST_RECURRENCE_ID_TODAY, $recurrenceId, $ttlInSeconds);
    }

    private function getLastRecurrenceIdProcessedToday(): int
    {
        return Cache::get(LogKeys::CRON_NOTIFY_EXPIRED_BOLETO_PIX_LAST_RECURRENCE_ID_TODAY, 0);
    }

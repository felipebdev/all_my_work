...
class PaymentRepository extends BaseRepository implements PaymentRepositoryInterface
{
    ...
    public function reportTransactionSale(string $platformId, SaleReportFilter $filters) {
        ...
        return $this->model
            ->select(
                DB::raw(
                    ...
                    CASE payments.type
                         ...
                         WHEN 'R' THEN CONCAT(recurrences.current_charge, '/', IF(plans.charge_until = 0, '∞', plans.charge_until))
            ...
            ->when($filters->paymentType, function ($query, $typePaymentFilter) {
    ...
    public function totalBilling(
        ...
        return $totalPaid + $totalPendingPixOrBankSlip + $totalPendingNoLimit;
    }
    ...
    public function graphTransactionByStatus(
        ...
        // no limit to receive (pending)
        $toReceive = $this->model
            ->select(DB::raw('
                SUM(payments.customer_value) AS value,
                "A Receber"
            '))
            ->where('payments.status', 'pending')
            ->where('payments.type', 'U')
        ...
        return $this->model
            ->select(DB::raw("
                SUM(payments.customer_value) AS value,
                CASE WHEN payments.status = 'paid' THEN 'Paga'
                    WHEN payments.status = 'pending' THEN 'Pendente'
                    WHEN payments.status = 'canceled' THEN 'Cancelada'
                    WHEN payments.status = 'chargeback' THEN 'Chargeback'
                END AS name
            "))
            ->whereRaw(DB::raw('
                payments.status in (
     ...
     * @inheritdoc
     *
     * This specific implementation is newest-first sorted
     */
    public function getPendingPaymentsOfPlansAndSubscriber(string $platformId, array $planIds, int $subscriberId)
    {
        return $this->model->select([
            ...
            ->where([
                'payments.platform_id' => $platformId,
                'payments.subscriber_id' => $subscriberId,
                'payments.status' => Payment::STATUS_PENDING,
            ])
            ->whereIn('payment_plan.plan_id', $planIds)
            ->whereIn('payments.type_payment', ['boleto', 'pix'])
            ->orderBy('payments.created_at', 'DESC')
            ->groupBy('plan_id')
            ->get();
    }

    public function getByOrderCodeAndSubscriberMail(string $orderCode, string $email) 
    {
        return $this->model
            ->select('payments.id', 'payments.platform_id', 'subscribers.name', 
                     'subscribers.email', 'payments.two_factor_code', 'two_factor_expires_at')
            ->join('subscribers', 'subscribers.id', '=', 'payments.subscriber_id')
            ->where([
                'payments.order_code' => $orderCode,
                'subscribers.email' => $email
            ])
            ->first();
    }

    public function putTwoFactorCode(int $id) 
    {
        $pin = mt_rand(100000, 999999);
        $val = $this->model->where('payments.id', $id)
                ->update([
                    'payments.two_factor_code' => $pin,
                    'payments.two_factor_expires_at' => Carbon::now()->addMinutes(30)
                ]);
        return $pin;
    }

...
class SubscriberImportQueue implements ShouldQueue
{
                ...
                foreach ($sheet->getRowIterator() as $row) {
                    if ($x >= 1) { // First Line ALWAYS is IGNORED
                        ...
                        if ($subscriber) {
                            ...
                            $plan = $this->hasThisPlan($subscriber->id, $this->plan_id);
                            if ($plan) {
                            ...
                            $subscriber->password = Hash::make(keygen(12));
    ...
    private function hasThisPlan($subscriberId, $planId)
    {
        $plan = Subscription::where('subscriber_id', $subscriberId)->where('plan_id', $planId)->first();
        return $plan ?? false;
    }

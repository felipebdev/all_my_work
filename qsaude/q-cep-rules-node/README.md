...
## ☕ Clonando e Instalando `<q-cep-rules-node>`

Para clonar o repositório `<q-cep-rules-node>`, siga estas etapas:

```bash
git clone <vcs>/q-cep-rules-node.git
```

Para instalar as dependências `<q-cep-rules-node>`, siga estas etapas:

```bash
npm run install
```

## ⚙️ Configurando ambiente `<q-cep-rules-node>`
...
## 🚀 Usando `<q-cep-rules-node>`

Para usar `<q-cep-rules-node>`, siga estas etapas:
